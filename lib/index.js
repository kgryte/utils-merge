	...
	if ( arguments.length ) {
		...
		if ( opts.hasOwnProperty( 'level' ) ) {
			...
			if ( !isPositiveInteger( level ) ) {
				throw new TypeError( 'invalid option. Level option must be a positive integer. Option: `' + level + '`.' );
			}
		}
		if ( opts.hasOwnProperty( 'copy' ) ) {
			copy = opts.copy;
			if ( !isBoolean( copy ) )  {
				throw new TypeError( 'invalid option. Copy option must be a boolean primitive. Option: `' + copy + '`.' );
			}
		}
		if ( opts.hasOwnProperty( 'override' ) ) {
			override = opts.override;
			if ( !isBoolean( override ) && !isFunction( override ) ) {
				throw new TypeError( 'invalid option. Override option must be either a boolean primitive or a function. Option: `' + override + '`.' );
			}
		}
		if ( opts.hasOwnProperty( 'extend' ) ) {
			extend = opts.extend;
			if ( !isBoolean( extend ) ) {
				throw new TypeError( 'invalid option. Extend option must be a boolean primitive. Option: `' + extend + '`.' );
			}
		}
	}
	return merge;
